...
class CMDInterface(Interface):
    ...
    def run(self):
        while True:
            ...
            if msg_code == OpeningMessage.EXIT:
            ...
            elif msg_code == OpeningMessage.CREATE:
                description = input("Add a task description:\t").strip()
                while True:
                    try:
                        priority_input = int(input("Add a task priority (1:Low, 2:Medium, 3:High):\t"))
                ...
                try:
                    task_number = int(input("Choose the number of the task you want to update (0 to cancel): "))
                    if task_number == 0:
                        continue
                    task_to_update = dict(self.todo.get_all_tasks()).get(task_number)
